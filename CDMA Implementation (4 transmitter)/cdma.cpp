...
int * transmitter::send_data_based_on_walsh_code()
{
	...
	if (rand() % 2 == 0) {
		...
		if (data_array[flag_for_last_success] == 0) {
			result[0] = Reverse(temp1);
			result[1] = Reverse(temp2);
			result[2] = Reverse(temp3);
			result[3] = Reverse(temp4);
...
void Shared_channel_TDMA::channel_engine()
{
	...
	while (ramain_data_for_all_tranmitters() > 0) {
		...
		my_result = new int[4];
		int* local_result;
		local_result = new int[4];

		while (local_cap > 0) {
			for (int i = 0; i < 4; i++) {
				if (transmitters_array[i].ramain_data() > 0) {
...
int main() {
	...
	Shared_channel_TDMA single_channel(30, A, B, C, D);
	cout << "main ..............." << endl;
